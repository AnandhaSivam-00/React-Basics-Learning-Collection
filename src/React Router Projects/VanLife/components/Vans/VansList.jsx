...
export const vansLoader = async ({ request }) => {
    ...
    return {
        vans: getVans() // Return the promise directly
    };
}
...
const VansList = () => {
    ...
    const renderVans = (vansData) => {
        const filteredVansDetails = !filter 
            ? vansData 
    ...
    return (
        <section className='container-fluid' style={{ marginTop: "100px" }}>
            ...
            <Suspense fallback={<p className="text-center my-5">Loading vans...</p>}>
                <Await resolve={vans}>
                    {renderVans}
                </Await>
            </Suspense>
